      ...
      >
        <template slot="operation" slot-scope="text, record, index">
          <div class="editable-row-operations">
            <span>
              <a @click="() =>score(record.scalePaperId)">查看/修改评分</a>
            </span>
          </div>
        </template>
        <!-- 操作 结束 -->
...
export default {
  ...
  mounted() {
    this.fetch({
      ...
      data: { examinationPaperId: this.$route.query.examinationPaperId }
    });
  },
  ...
  methods: {
    ...
    fetch(params = {}) {
      ...
      this.loading = true;
      reqwest({
        url: this.serverUrl + "paper/scale/list", //获取量表答卷列表
        headers: {
        ...
        values => {
          ...
          if (values.retCode === "000000") {
            ...
            for (var i = 0; i < values.data.list.length; i++) {
              ...
              values.data.list[i].useTime =
                values.data.list[i].useTime + "分钟";
            }
    ...
    score(scalePaperId) {
      this.$router.push({
        path: "/Home/myReportAnswer/myScaleAnswer/answerJudge",
        // patients:patientId
        query: { scalePaperId: scalePaperId }
      });
    },
